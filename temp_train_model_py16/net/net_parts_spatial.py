...
class Conv2times(nn.Module):
    def __init__(self,
        ...
        self.conv2times = nn.Sequential(
            BasicConv2d(in_channels, out_channels, kernel_size, padding, relu,
                        bn, **kwargs),
            BasicConv2d(out_channels, out_channels, kernel_size, padding, relu,
                        bn, **kwargs))
...
class Downsample(nn.Module):
    def __init__(self,
        ...
        self.spatial_bias = torch.nn.Parameter(
            torch.randn(1, out_channels, H, W).cuda().requires_grad_())
...
if __name__ == '__main__':
    down = Downsample(2, 22, 44, 3, 1, H=34, W=36).cuda()
    up = Upsample(2, 44, 22, 3, 1, H=69, W=73).cuda()
    print(down)
    print(up)
    for name, item in down.named_parameters():
        print("the name is ", name)
        #print("the parameters is ",item)
    x = torch.rand(8, 22, 69, 73).cuda()
    xx = torch.rand(8, 22, 69, 73).cuda()
    t = down(x).cuda()
    x_hat = up(t, xx)
    print(t.shape)
    print(x_hat.shape)
