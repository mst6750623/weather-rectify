...
class Validate(nn.Module):
    ...
    def initialize(self, unet_path):
        ...
        self.unet.load_state_dict(unet_ckpt['unet_state_dict'])
        self.timeReflect.load_state_dict(unet_ckpt['time_reflect_state_dict'])
        self.unet.eval()
    ...
    def simple_validate(self):
        ...
        for i, iter in enumerate(tqdm(self.data_iter, desc="validating: ")):
            with torch.no_grad():
                ...
                pred_temperature = self.unet(newinput)
